...
*   **错题本**：使用 IndexedDB 持久化存储答题历史，记录每道题的答对/答错次数，并支持“待复习 / 全部 / 已刷回”三种视图、按难度分组折叠展示、错题一键进入做题模式重刷；在做题模式下将错题重刷做对后，会自动从待复习列表中移除。
...
*   **UI 布局与响应式体验优化**：
    *   **悬浮倒计时球**：复刻官方高亮圆形计时器样式，悬浮于右侧，临近时间具有变红呼吸警告效果，做题时间尽在掌握。
    *   **面板自适应缩放**：解决小屏幕遮挡问题，面板会根据当前浏览器宽度自动计算比例并进行动态缩放（依靠 CSS `zoom` 引擎级支持）。
    *   **功能区动静分离布局**：将“做题统计”与“棋书练习进度”等高频追踪数据硬顶置于面板上方，下方的“设置/错题本查询”放入内置滚动区单独滚动。杜绝了配置面板展开时将核心统计数据挤出视野的问题，数据查看更稳固！
