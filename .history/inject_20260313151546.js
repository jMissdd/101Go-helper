(function() {
    ...
    function hasProblemShape(payload) {
    ...
    function scanGlobalVariables() {
        let alpineQqdata = null;
        try {
            if (typeof Alpine !== 'undefined') {
                const store = Alpine.store('qipan');
                if (store && store.qqdata) {
                    alpineQqdata = store.qqdata;
                    registerAlpineWatchers(store);
                }
            }
        } catch (e) {}

        if (hasProblemShape(alpineQqdata)) {
            let plainData = null;
            try {
                plainData = JSON.parse(JSON.stringify(alpineQqdata));
            } catch (e) {
                plainData = alpineQqdata;
            }

            const currentProblemId = alpineQqdata.publicid || alpineQqdata.id || 'unknown';
            ...
            if (isNewProblem || isFirstLoad) {
                window._problemResultCache = {};
                window._problemState = 'PENDING';
                window._pendingSetTime = Date.now();
                ...
                answerResult = readAnswerResultFromStore(alpineQqdata, currentProblemId);
            }
